...
batch_sizes=(64)  # Add your specific batch size values
epochs=(40)
patiences=(10)
                        ...
                        for lr in "${learning_rates[@]}"; do
                            for seq_length in "${seq_lengths[@]}"; do
                                for patience in "${patiences[@]}"; do
                                    ...
                                    python your_script.py "train" --model $model --num_layers $num_layer --hidden_dim $hidden_dim --loss $loss --batch_size $batch_size --epoch $epoch --lr $lr --seq_length $seq_length --patience $patience --save_model
                                done
                            done
                        done
                    done
